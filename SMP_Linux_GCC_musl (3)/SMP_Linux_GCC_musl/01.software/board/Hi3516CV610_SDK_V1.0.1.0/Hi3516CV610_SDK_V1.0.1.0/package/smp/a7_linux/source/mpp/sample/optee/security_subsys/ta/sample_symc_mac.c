...
static td_s32 sample_symc_mac_run(symc_mac_data_t *data)
{
    ...
    /* 1. cipher init */
    sample_chk_expr_return(ot_mpi_cipher_symc_init(), TD_SUCCESS);
    ...
    /* 5. set mac attr */
    sample_chk_expr_goto(ot_mpi_cipher_mac_start(&symc_handle, &data->mac_attr), TD_SUCCESS, KEYSLOT_DESTROY);

    /* 6. mac process */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_mpi_cipher_mac_update(symc_handle, &src_buf, data->message_length), TD_SUCCESS,
        CIPHER_DESTROY);

    /* 7. finish mac calculation and get the mac value */
    sample_chk_expr_goto(ot_mpi_cipher_mac_finish(symc_handle, mac, &mac_length), TD_SUCCESS, CIPHER_DESTROY);
