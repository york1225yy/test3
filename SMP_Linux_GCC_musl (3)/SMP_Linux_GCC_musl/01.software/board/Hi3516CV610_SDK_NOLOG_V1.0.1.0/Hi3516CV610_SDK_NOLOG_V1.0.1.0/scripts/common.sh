...
run_command_progress()
{
    ...
    echo_info "run_command_progress: '$1'"
    [ -z "$steps" ] && { steps=1; }
...
run_command_progress_float()
{
    ...
    echo_info "run_command_progress_float: '$1'"
...
echo_info "unpacking $cfg_moddir"
...
make_dirs()
{
    ...
    [ -z "$1" ] && { echo_err "make_dirs mast have a prefix dir!"; return 1; }
    $MKDIR $1 -p
...
{
    local subdir=""
    local dir_a=$(dirname $1/stub)
    local dir_b=$(dirname $2/stub)

    while [ "$dir_a" != "$dir_b" ] && [ "$dir_a" != "." ] && [ "$dir_a" != "/" ]
    do
        if [ -z "$subdir" ] ; then
            subdir=$(basename $dir_a)
        else
            subdir=$(basename $dir_a)$3/$subdir
        fi
        dir_a=$(dirname $dir_a)
    done
...
write_rootfs_level()
{
    ...
    [ -n "$(grep "^\[[0-9A-Za-z][0-9A-Za-z\-]*\]	$1$" < $rlevel_config)" ] && { \
        echo_warn "$rlevel_config already have item '$1'"
        return 0;
    }
...
remove_all_cvsdir()
{
    ! [ -d "$1" ] && { echo_warn "'$1' not found when remove 'CVS' directories."; return ; }

    echo_info "Remove: 'CVS' directories in $1"
...
strip_lib()
{
    ...
    do
        ...
        echo_info "strip not really done: $file"
    done
}
...
install_extern_kmod()
{
    ...
    do
        ...
        [ -f "$HCM_DESTDIR/$HCM_KERNEL_INSTALL_RESOURCE/$iek_dest_module" ] && \
            { echo_warn "Extern module $iek_extmod conflict: $iek_dest_module"; sleep 1; }
    ...
    echo_info "Generating modules dependency ..."
...
function string_to_varname()
{
    echo "$1" | sed 's/[^a-zA-Z0-9_]/_/g'
}

function patchset_get_param()
{
    echo "$1" | cut -d')' -f1 | sed 's/[\(\|]/ /g'
}

function patchset_get_name()
{
    echo "$file" | cut -d')' -f2
}

# $1: tgz package
function run_command_progress_tar()
{
    local tar_file=$1
    local dir_name=${tar_file%.*}
    dir_name=${dir_name#*/}

    total_size=$(gzip -l $tar_file | tail -n 1 | awk '{print $2}')
    block_size=$(expr $total_size / 51200 + 1)
    tar --blocking-factor=$block_size --checkpoint=1 --checkpoint-action=exec='printf "%3d%% of %32s unpacked\r" $TAR_CHECKPOINT $TAR_ARCHIVE' -xzf package/${dir_name}.tgz
    printf "%3d%% of %32s unpacked\n" 100 "package/${dir_name}.tgz"
}
SDK_CHIP=hi3516cv610
OS_TYPE=smp_linux
