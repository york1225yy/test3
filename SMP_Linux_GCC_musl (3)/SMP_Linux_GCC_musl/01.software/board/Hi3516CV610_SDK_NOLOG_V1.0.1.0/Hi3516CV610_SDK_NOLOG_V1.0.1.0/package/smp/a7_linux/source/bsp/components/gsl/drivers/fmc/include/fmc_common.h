...
#define fmc_print(fmt, args...)  printf(fmt, ##args)
#else
#define fmc_print(fmt, args...)
#endif
...
#define db_msg(_fmt, arg...) \
	fmc_print("%s(%d): " _fmt, __func__, __LINE__, ##arg)

#define db_bug(fmt, args...) \
	do { \
		fmc_print("%s(%d): BUG: " fmt, __FILE__, __LINE__, ##args); \
		while (1); \
	} while (0)

#define FMC_INFO    1

#define BT_DBG		1 /* Boot init debug print */
#define ER_DBG		0 /* Erase debug print */
#define WR_DBG		0 /* Write debug print */
#define RD_DBG		1 /* Read debug print */
...
#define DTR_DB		1 /* 4DTR debug print */
