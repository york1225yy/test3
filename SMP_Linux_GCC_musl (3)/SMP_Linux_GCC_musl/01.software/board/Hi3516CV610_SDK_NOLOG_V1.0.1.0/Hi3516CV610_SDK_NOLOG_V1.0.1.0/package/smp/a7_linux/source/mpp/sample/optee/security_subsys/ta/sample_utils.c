...
td_void cipher_free(const crypto_buf_attr *buf_attr, const void *virt_addr)
{
    if (buf_attr->phys_addr != TD_NULL && virt_addr != TD_NULL) {
        mpi_sys_mmz_free((td_phys_addr_t)buf_attr->phys_addr, (td_void *)virt_addr);
    }
}
...
td_s32 cipher_alloc(crypto_buf_attr *buf_attr, void **virt_addr, unsigned int size)
{
    ...
    ret = mpi_sys_mmz_alloc(&phys_addr, virt_addr, "SAMPLE Buffer", "SMMZ2", size);
    if (ret != TD_SUCCESS) {
        sample_err("mpi_sys_mmz_alloc failed\n");
        return TD_FAILURE;
    }
...
td_s32 get_random_data(td_u8 *buffer, td_u32 size)
{
    ...
    ret = ot_mpi_cipher_trng_get_multi_random(size, buffer);
    return ret;
}
