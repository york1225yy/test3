...
static td_void sample_ive_handle_sig(td_s32 singal)
{
    if (singal == SIGINT || singal == SIGTERM) {
        switch (*g_cmd_argv[1]) {
            ...
            case '5': {
                sample_ive_warp_persp_trans_handle_sig();
                break;
                }
            case '6': {
...
static td_void sample_ive_usage(const td_char *name)
{
    ...
    printf("\t 6)Occlusion detected.(VI->VPSS->IVE->VENC).\n");
    printf("\t 7)Motion detected.(VI->VPSS->IVE->VGS->VENC).\n");
    printf("\t 8)Resize.(FILE->IVE->FILE).\n");
}
...
static td_s32 sample_ive_case_with_two_arg(char *argv[])
{
    ...
    td_s32 len = (td_s32)strlen(argv[OT_SAMPLE_IVE_ARG_IDX_TWO]);
    switch (*argv[1]) {
        case '0': {
