...
static td_s32 run_sm2_generate_key_sample(ecc_data *data)
{
    ...
    /* 1. pke init */
    sample_chk_expr_return(ot_mpi_cipher_pke_init(), TD_SUCCESS);

    /* 2. generate ecc key */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_key(data->curve_type, TD_NULL, &priv_key, &pub_key), TD_SUCCESS,
        __PKE_DEINIT__);

    /* 3. check if the dot is on the specific curve */
    sample_chk_expr_goto(ot_mpi_cipher_pke_check_dot_on_curve(data->curve_type, &pub_key, &is_on_curve), TD_SUCCESS,
        __PKE_DEINIT__);
    sample_chk_expr_goto_with_ret(is_on_curve == TD_TRUE, TD_TRUE, ret, TD_FAILURE, __PKE_DEINIT__);

__PKE_DEINIT__:
    ot_mpi_cipher_pke_deinit();
...
static td_s32 run_sm2_ecdh_sample(ecc_data *data)
{
    ...
    /* 2. get key */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_key(data->curve_type, TD_NULL, &priv_key1, &pub_key1), TD_SUCCESS,
        __PKE_DEINIT__);

    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_key(data->curve_type, TD_NULL, &priv_key2, &pub_key2), TD_SUCCESS,
        __PKE_DEINIT__);

    /* 3. get ecdh key */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_ecdh_key(data->curve_type, &pub_key2, &priv_key1, &shared_key1),
        TD_SUCCESS, __PKE_DEINIT__);

    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_ecdh_key(data->curve_type, &pub_key1, &priv_key2, &shared_key2),
        TD_SUCCESS, __PKE_DEINIT__);
...
static td_s32 run_sm2_sign_sample(ecc_data *data)
{
    ...
    /* 1. pke init */
    sample_chk_expr_goto(ot_mpi_cipher_pke_init(), TD_SUCCESS, __EXIT_FREE__);

    /* 2. get key */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_key(data->curve_type, TD_NULL, &output_priv_key, &output_pub_key),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 3. Check Dot */
    sample_chk_expr_goto(ot_mpi_cipher_pke_check_dot_on_curve(data->curve_type, &output_pub_key, &is_on_curve),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 4. DSA Hash */
    sample_chk_expr_goto(ot_mpi_cipher_pke_sm2_dsa_hash(&sm2_id, &output_pub_key, &msg, &input_hash),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 4. ECC Sign. */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecdsa_sign(data->curve_type, &output_priv_key, &input_hash, &output_sig),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 5. ECC Verify. */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecdsa_verify(data->curve_type, &output_pub_key,
...
static td_s32 run_sm2_crypto_sample(ecc_crypto_data *data)
{
    ...
    /* 1. Pke Init */
    sample_chk_expr_goto(ot_mpi_cipher_pke_init(), TD_SUCCESS, __EXIT_FREE_CHECK__);

    /* 2. Generate Key */
    sample_chk_expr_goto(ot_mpi_cipher_pke_ecc_gen_key(data->curve_type, TD_NULL, &priv_key, &pub_key),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 3. Check Dot */
    sample_chk_expr_goto(ot_mpi_cipher_pke_check_dot_on_curve(data->curve_type, &pub_key, &is_on_curve),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 4. SM2 Public Encrypt */
    sample_chk_expr_goto(ot_mpi_cipher_pke_sm2_public_encrypt(&pub_key, &plain_text, &cipher_text),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 5. SM2 Private Decrypt */
    sample_chk_expr_goto(ot_mpi_cipher_pke_sm2_private_decrypt(&priv_key, &cipher_text, &check_text),
        TD_SUCCESS, __PKE_DEINIT__);
