...
static td_s32 sample_ive_lk_param_init(ot_sample_ive_st_lk_inf *lk_info, ot_size src_size, td_u8 max_level)
{
    ...
    /* Init prev pts */
    size = (td_u32)sizeof(ot_svp_point_s25q7) * OT_SAMPLE_IVE_LK_MAX_POINT_NUM;
    ...
    /* Init status */
    size = (td_u32)sizeof(td_u8) * OT_SAMPLE_IVE_LK_MAX_POINT_NUM;
    size = sample_common_ive_calc_stride(size, OT_IVE_ALIGN);
    ...
    /* Init err */
    size = (td_u32)sizeof(td_u9q7) * OT_SAMPLE_IVE_LK_MAX_POINT_NUM;
    size = sample_common_ive_calc_stride(size, OT_IVE_ALIGN);
...
static td_s32 sample_ive_st_param_init(ot_sample_ive_st_lk_inf *lk_info, ot_size src_size, ot_size pyr_size)
{
    ...
    size = (td_u32)sizeof(td_u32) * sample_common_ive_calc_stride(src_size.width, OT_IVE_ALIGN) * src_size.height +
...
static td_s32 sample_ive_st_open_dst_file(ot_sample_ive_st_lk_inf *lk_info)
{
    td_s32 ret = TD_FAILURE;
    td_char path[PATH_MAX + 1] = {0};
    /* open dst file */
    sample_svp_check_exps_return(realpath("./data/output/stlk", path) == TD_NULL, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "invalid file!\n");
    ...
    sample_svp_check_exps_return(lk_info->fp_dst_status == TD_NULL, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Open file failed!\n");
    ...
    sample_svp_check_exps_return(lk_info->fp_dst_err == TD_NULL, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Open file failed!\n");
    ...
    sample_svp_check_exps_return(lk_info->fp_dst_next_pts == TD_NULL, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Open file failed!\n");
    return TD_SUCCESS;
}
/*
 ...
 */
static td_s32 sample_common_ive_write_stlk_file(ot_svp_mem_info *lk_out, FILE *fp, td_u16 points_num)
{
    td_u8 *ptr_tmp = TD_NULL;
    td_s32 ret = OT_ERR_IVE_NULL_PTR;

    sample_svp_check_exps_return(lk_out == TD_NULL, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
    ...
    ptr_tmp = sample_svp_convert_addr_to_ptr(td_u8, lk_out->virt_addr);
