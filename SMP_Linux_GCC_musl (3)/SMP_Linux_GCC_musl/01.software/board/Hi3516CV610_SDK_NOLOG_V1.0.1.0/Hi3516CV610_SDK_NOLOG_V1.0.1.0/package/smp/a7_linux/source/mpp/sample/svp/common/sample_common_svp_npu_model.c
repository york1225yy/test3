    ...
    td_bool is_input, td_u32 *total_line_num, td_u32 *line_byte_num)
{
    ...
    *line_byte_num = (td_u32)dims.dims[dims.dim_count - 1] *
    ...
    *total_line_num = 1;
    ...
    if ((task->cfg.total_t != 0) && (idx == 0)) {
        sample_svp_check_exps_return(task->cfg.total_t > dims.dims[0], TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "total t(%u) can't be greater than max total t(%lld)!\n", task->cfg.total_t, dims.dims[0]);
...
static td_s32 sample_svp_npu_read_model(const td_char *model_path, td_u32 model_index, td_bool is_cached)
{
    ...
    g_svp_npu_model[model_index].model_mem_size = (td_ulong)ftell(fp);
    ...
    ret = (td_s32)fread(g_svp_npu_model[model_index].model_mem_ptr, g_svp_npu_model[model_index].model_mem_size, 1, fp);
...
td_s32 sample_common_svp_npu_create_task_buf(sample_svp_npu_task_info *task)
{
    ...
    sample_svp_check_exps_return(num != g_svp_npu_model[task->cfg.model_idx].input_num - SAMPLE_SVP_NPU_EXTRA_INPUT_NUM,
        TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "num of data buffer(%zu) should be %u when create task buf!\n",
...
td_s32 sample_common_svp_npu_create_work_buf(sample_svp_npu_task_info *task)
{
    ...
    sample_svp_check_exps_return(num != g_svp_npu_model[task->cfg.model_idx].input_num - 1,
        TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "num of data buffer(%zu) should be %u when create work buf!\n",
...
td_s32 sample_common_svp_npu_set_dynamic_batch(const sample_svp_npu_task_info *task)
{
    ...
    if (task->cfg.total_t != 0) {
        ...
        sample_svp_check_exps_return(ret != SVP_ACL_SUCCESS, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
            ...
            g_svp_npu_model[task->cfg.model_idx].model_id, ret);
        sample_svp_trace_info("set total_t success!\n");
    }
    ...
    td_bool is_input)
{
    ...
    for (i = 0; i < data_num; i++) {
        ...
        for (line = 0; line < task->cfg.dynamic_batch_num * total_line_num; line++) {
            ret = (td_s32)fwrite(data, line_byte_num, 1, fp);
            if (ret != 1) {
                fclose(fp);
...
static td_bool sample_svp_npu_check_valid_rect(const ot_point *point)
{
    if (point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x == point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x) {
        return TD_FALSE;
    }
    if (point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y == point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].y) {
        return TD_FALSE;
    }
    return TD_TRUE;
}
    ...
    const ot_video_frame_info *proc_frame, const ot_video_frame_info *show_frame, ot_sample_svp_rect_info *rect_info)
{
    ...
    for (i = 0, j = 0; i < rect_info->num; i++) {
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x = (td_u32)((td_float)x_min[roi_offset] /
            proc_frame->video_frame.width * show_frame->video_frame.width) & (~1);
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y = (td_u32)((td_float)y_min[roi_offset] /
            proc_frame->video_frame.height * show_frame->video_frame.height) & (~1);

        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x = (td_u32)((td_float)x_max[roi_offset] /
            proc_frame->video_frame.width * show_frame->video_frame.width) & (~1);
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].y =
            rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y;

        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_BOTTOM].x =
            rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x;
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_BOTTOM].y = (td_u32)((td_float)y_max[roi_offset] /
            proc_frame->video_frame.height * show_frame->video_frame.height) & (~1);

        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].x =
            rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x;
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_BOTTOM].y =
            rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_BOTTOM].y;
        roi_offset++;
        if (sample_svp_npu_check_valid_rect(rect_info->rect[j].point) == TD_TRUE) {
            j++;
        }
    }
    rect_info->num = j;
    ...
    const ot_video_frame_info *show_frame, ot_sample_svp_coords_info *coords_info)
{
    ...
    c = (td_u32)dim.dims[1];
    h = (td_u32)dim.dims[SAMPLE_SVP_NPU_H_DIM_IDX];
    w = (td_u32)dim.dims[SAMPLE_SVP_NPU_W_DIM_IDX];
    ...
    for (i = 0; i < c; i++) {
        ...
        if (px > 1 && px < (td_s32)w - 1 && py > 1 && py < (td_s32)h - 1) {
            double diffx = (double)(out[i * (h * stride) + (td_u32)py * stride + px + 1] -
                out[i * (h * stride) + (td_u32)py * stride + px - 1]);
            double diffy = (double)(out[i * (h * stride) + ((td_u32)py + 1) * stride + px] -
                out[i * (h * stride) + (td_u32)(py - 1) * stride + px]);
    ...
    const sample_svp_npu_task_info *task)
{
    ...
    for (i = 0; i < threshold_num; i++) {
        ...
        sample_svp_check_exps_return(size < SAMPLE_SVP_NPU_THRESHOLD_NUM * sizeof(td_float), TD_FAILURE,
            SAMPLE_SVP_ERR_LEVEL_ERROR, "get size(%zu) is less than %u!\n",
    ...
    ot_size *pic_size)
{
    ...
    sample_svp_check_exps_return(ret != SVP_ACL_SUCCESS, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "get num dims failed!\n");
    pic_size->height = (td_u32)dims.dims[dims.dim_count - SAMPLE_SVP_NPU_H_DIM_IDX]; // NCHW
    pic_size->width = (td_u32)dims.dims[dims.dim_count - 1]; // NCHW
    return TD_SUCCESS;
}
